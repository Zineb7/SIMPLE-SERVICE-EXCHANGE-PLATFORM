			...
			if ($post_status_qry) {
				...
				if (isset($_POST['accept'])) {
					if ($post_status == 4) {
						...
						$seekerCoinsDeduction = $coin_value * $coinPercentage; // 25% deduction
						...
						$providerCoinsGain = $seekerCoinsDeduction + $coin_value;
						$update_provider_balance_qry = $conn->query("UPDATE member_list 
																  SET coin = coin + {$providerCoinsGain} 
																  WHERE id = '{$receiverId}'");
						...
						if ($update_seeker_balance_qry && $update_provider_balance_qry && $insert_query) {
							?>
							<script>
								alert_toast("Accepted as Provider. Deducted 25% from Seeker. Provider gained <?php echo $providerCoinsGain; ?> coins.", 'success');
							</script>
							<?php
						} else {
							?>
						...
						// Define the coin percentage variable
						$coinPercentage = 0.25;
						// Deduct 25% from Receiver's balance
						$receiverCoinsDeduction = $coin_value * $coinPercentage; // 25% deduction
	
						// Update Provider's balance
						$update_provider_balance_qry = $conn->query("UPDATE member_list 
																	 SET coin = coin - {$receiverCoinsDeduction} 
																	 WHERE id = '{$receiverId}'");
						...
						if ($update_provider_balance_qry && $update_receiver_balance_qry && $insert_query) {
							?>
							<script>
								alert_toast("Accepted as Seeker. Deducted 25% from Provider. Receiver gained <?php echo $receiverCoinsDeduction; ?> coins.", 'success');
							</script>
							<?php
						} else {
							?>
					...
					// After performing the decline action, you can display a message
					?>
					<script>
